...
class DfacSpaceExtractor_max(nn.Module):
    def __init__(self, local_observation_space, global_observation_space, control, **kwargs):
        super().__init__()
    ...
    def forward(self, local_obs, global_obs, step):
        return np.concatenate([[local_obs[self.control]], global_obs["freewind_measurements"].flatten()],0)


class DfacSpaceExtractor_track(nn.Module):
    def __init__(self, local_observation_space, global_observation_space, control, track_power):
        super().__init__()
        self.control = control
    ...
    def forward(self, local_obs, global_obs, step):
        return np.concatenate([[local_obs[self.control]], global_obs["freewind_measurements"].flatten(), [self.track_power[step]]], 0)
