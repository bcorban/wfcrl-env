...
def meshing( Wp, plotMesh=False, PrintGridMismatch=False ):
    ...
    for i in range(n_turbines):
        ...
        x_center = np.argmin(np.abs(ldx - Wp.turbine.Crx[i])) 
        ...
        y_center = np.argmin(np.abs(ldy - Wp.turbine.Cry[i]))
        ...
        yline_i = np.arange(ML_prim, MR_prim + 1)
        ylinr_i_v = np.arange(ML_prim, MR_prim + 2)
        yline.append(yline_i)          # turbine cells for primary grid
        ylinev.append(ylinr_i_v)       # turbine cells for secondary grid
        
        turb_lines_i = np.zeros((len(ylinr_i_v),4)).astype(np.int32)
        turb_lines_i[:-1,0] = xline_i
        turb_lines_i[:-1,1] = yline_i
        turb_lines_i[:,2] = xline_i
        turb_lines_i[:,3] = ylinr_i_v
        turb_lines.append(turb_lines_i)
        print(f'turb{i}: mesh.xline_i={xline_i}, mesh.yline_i={yline_i}, ylinr_i_v={ylinr_i_v}')
    ...
    Wp.mesh.x_center = x_center
    Wp.mesh.y_center = y_center
